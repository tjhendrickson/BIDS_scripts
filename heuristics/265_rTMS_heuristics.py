...
def infotodict(seqinfo):
    ...
    for idx, s in enumerate(seqinfo):
        ...
        elif (s.dim4 == 1040) and (('REST' in s.protocol_name)):
            info[rest].append({'item': s.series_id})
        elif (s.dim4 == 1) and (('REST' in s.protocol_name)):
            if s_next.dim4 == 1040:
                info[sbref_rest].append({'item': s.series_id})
        elif (s.dim4 == 100):
            if 'KET_TASK1_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'KET1'})
            elif 'KET_TASK2_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'KET2'})
            elif 'KET_TASK3_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'KET3'})
            elif 'KET_TASK4_PA' in s.protocol_name:
                info[task].append({'item': s.series_id, 'acq': 'KET4'})
        elif (s.dim4 == 1):
            if ('KET_TASK1_PA' in s.protocol_name):
                if s_next.dim4 == 100:
                    info[sbref_task].append({'item': s.series_id, 'acq':'KET1' })
            elif ('KET_TASK2_PA' in s.protocol_name):
                if s_next.dim4 == 100:                
                    info[sbref_task].append({'item': s.series_id, 'acq': 'KET2'})
            elif ('KET_TASK3_PA' in s.protocol_name):
                if s_next.dim4 == 100:
                    info[sbref_task].append({'item': s.series_id, 'acq': 'KET3' })
            elif ('KET_TASK4_PA' in s.protocol_name):
                if s_next.dim4 == 100:
                    info[sbref_task].append({'item': s.series_id, 'acq': 'KET4'})
        ...
        elif (s.dim4 == 159) and (('DWI_79dir_b1000_2000_REV_PA' in s.protocol_name) or ('DWI_79dir_b1000_2000_AP' in s.protocol_name)):
            info[dwi].append({'item': s.series_id, 'acq': 'AP'})
        elif (s.dim4 == 159) and ('DWI_79dir_b1000_2000_PA' in s.protocol_name):
